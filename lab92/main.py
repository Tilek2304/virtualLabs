...
class LabTempCoeffApp(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Температурный коэффициент сопротивления металлов (α)")
        ...
        self.input_Tt = QLineEdit(); self.input_Tt.setPlaceholderText("T (°C) — конечная температура (ΔT = T - T0)")
    ...
    def apply_params(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения R0, T0 и (опционально) α.")
            return
        self.cal_widget.set_params(R0=R0, T0=T0, alpha=alpha)
        self.input_Rt.clear(); self.input_Tt.clear(); self.input_alpha_user.clear()
        self.lbl_feedback.setText("Параметры применены. Нажмите «Нагреть» для симуляции или введите свои измерения.")
        self._update_meter()
    ...
    def heat_step(self):
        ...
        self.lbl_feedback.setText(f"Пробирка нагрета на ΔT ≈ {dT:.2f} °C. Нажмите «Измерить» для имитации показаний.")
        self._update_meter()
