...
class LampWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        p.drawText(10, h - 10, f"Яркость {self.brightness*100:.0f}%")
...
class CircuitWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if hasattr(self, "P_lamp"):
            p.drawText(12, mid_y - 24, f"P лампы = {self.P_lamp:.3f} Вт")

# Главное приложение
class LabPowerApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораторная Мощность и работа тока в лампе")
        ...
        # правая панель
        right.addWidget(QLabel("<b>Мощность и работа тока в лампе</b>"))
        info = QLabel(
            "Настройте напряжение источника и сопротивление лампы.\n"
            "Нажмите Запустить чтобы включить лампу и начать отсчёт времени.\n"
            "Формулы P = U·I и A = P·t."
        )
        info.setWordWrap(True)
        right.addWidget(info)

        right.addWidget(QLabel("<b>Параметры</b>"))
        self.input_U = QLineEdit(); self.input_U.setPlaceholderText("U источник В")
        self.input_R = QLineEdit(); self.input_R.setPlaceholderText("R лампы Ом")
        self.input_Rint = QLineEdit(); self.input_Rint.setPlaceholderText("R внутреннее Ом")
        ...
        right.addSpacing(6)
        right.addWidget(QLabel("<b>Измерения и ответы</b>"))
        self.input_P = QLineEdit(); self.input_P.setPlaceholderText("P Вт — ваш расчёт")
        self.input_A = QLineEdit(); self.input_A.setPlaceholderText("A Дж — ваш расчёт")
        ...
        btn_check = QPushButton("Проверить P и A")
    ...
    def apply_params(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения U и R лампы (и опционально R внутреннее).")
            return
        ...
        # лампа яркость по мощности на лампе
        P_lamp = getattr(self.circuit, "P_lamp", 0.0)
        # нормируем яркость по разумному максимум
        b = min(1.0, P_lamp / max(0.1, self.circuit.U * self.circuit.I if self.circuit.I else 0.1))
        self.lamp.set_brightness(b)
        self.lbl_result.setText("Параметры применены. Нажмите Запустить для измерения времени.")

    def start(self):
        if self.circuit.I is None:
            QMessageBox.information(self, "Инфо", "Сначала задайте параметры и соберите цепь.")
            return
        if not self.running:
            self.timer.start(100)  # 0.1 с шаг
            self.running = True
            self.lbl_result.setText("Таймер запущен.")

    def stop(self):
        if self.running:
            self.timer.stop()
            self.running = False
            self.lbl_result.setText("Таймер остановлен.")
    ...
    def _tick(self):
        ...
        # обновляем лампу яркость динамически если нужно
        P_lamp = getattr(self.circuit, "P_lamp", 0.0)
        b = min(1.0, P_lamp / max(0.1, self.circuit.U * self.circuit.I if self.circuit.I else 0.1))
        self.lamp.set_brightness(b)
    ...
    def measure(self):
        ...
        QMessageBox.information(self, "Измерение", f"Показания: I = {I_meas:.3f} A, U = {U_meas:.2f} V\nP лампы = {P_lamp:.3f} W")
        # не заполняем поля P и A автоматически — ученик должен рассчитать
        self.lbl_result.setText("Приборы показали значения. Рассчитайте P и A и введите их.")
    ...
    def check_answers(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения P и A.")
            return
        if self.circuit.I is None:
            QMessageBox.information(self, "Инфо", "Сначала соберите цепь и измерьте приборы.")
            return
        # истинные значения
        ...
        lines = []
        if okP:
            lines.append("✅ P рассчитана верно.")
        else:
            lines.append(f"❌ P неверно. Правильная P ≈ {P_true:.3f} Вт (допуск ±{tol_P:.3f}).")
        if okA:
            lines.append("✅ A рассчитана верно.")
        else:
            lines.append(f"❌ A неверно. Правильная A ≈ {A_true:.3f} Дж (допуск ±{tol_A:.3f}).")
    ...
    def show_answers(self):
        ...
        self.lbl_result.setText("Показаны правильные значения P и A.")
