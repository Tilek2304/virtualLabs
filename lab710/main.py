...
class FrictionLab(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лаборатория: Сила трения (Fтр = μ·N)")
        ...
        left_layout.addWidget(QLabel("<b>Сила трения (Fтр = μ·N)</b>"))
        ...
        self.btn_apply_mass = QPushButton("Применить изменения в массе")
        self.btn_apply_mass.clicked.connect(self.apply_mass_change)
        left_layout.addWidget(self.btn_apply_mass)

        # Материал
        left_layout.addWidget(QLabel("Материал (коэф. трения μ)"))
        self.mu_select = QComboBox()
        self.mu_values = {"Дерево":0.4, "Металл":0.2, "Резина":0.8, "Лёд":0.05}
        ...
        self.input_Ftr = QLineEdit(); self.input_Ftr.setPlaceholderText("Fтр (Н)")
        ...
        self.btn_check = QPushButton("Проверить ответы"); self.btn_check.clicked.connect(self.check_answers)
        ...
        self.summary_label = QLabel("Среднее Fтр: —, σ: —, точек: 0")
        ...
        self.table.setHorizontalHeaderLabels(["t","m (кг)","μ","N (Н)","Fтр (Н)"])
    ...
    def apply_mass_change(self):
        ...
        QMessageBox.information(self,"Изменения применены",f"Масса изменена на {m} кг")
    ...
    def clear_measurements(self):
        ...
        self.summary_label.setText("Среднее Fтр: —, σ: —, точек: 0")
    ...
    def recalc_analysis(self):
        ...
        self.summary_label.setText(f"Среднее Fтр: {mean:.2f} Н, σ: {std:.2f}, точек: {len(vals)}")
    ...
    def check_answers(self):
        ...
        N = m*g
        F_true = mu*N
        P_true = N/0.01  # условная площадь 0.01 м²
    ...
    def export_csv(self):
        if not self.measurements: return
        path,_=QFileDialog.getSaveFileName(self,"Сохранить CSV","friction_session.csv","CSV Files (*.csv)")
        if not path: return
    ...
    def save_session_json(self):
        ...
        path,_=QFileDialog.getSaveFileName(self,"Сохранить JSON","friction_session.json","JSON Files (*.json)")
        if not path: return
