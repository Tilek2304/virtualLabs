...
class LabTempCoeffCopperApp(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Температурный коэффициент меди — α = ΔR / (R0·Δt)")
        ...
        right.addSpacing(6)
        right.addWidget(QLabel("<b>Управление нагревом</b>"))
        btn_heat_on = QPushButton("Включить нагрев")
        btn_heat_on.clicked.connect(self.heat_on)
        btn_heat_off = QPushButton("Выключить нагрев")
        btn_heat_off.clicked.connect(self.heat_off)
        btn_step = QPushButton("Быстрый шаг нагрева +ΔT")
