...
class LabRheostatApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык — Реостат менен агымдын күчүнүн чегилүүсү")
        ...
        # правая панель
        right.addWidget(QLabel("<b>Реостат менен агымдын күчүнүн чегилүүсү</b>"))
        info = QLabel("Реостат сырткасын жылдырууда I(R) көз карандылыгын байкаңыз.\n"
                      "Ом мыйзамы: I = U / (Rфикс + Rрео).")
        ...
        self.input_I = QLineEdit(); self.input_I.setPlaceholderText("I (А) — сиздин эсептөөңүз учурдагы R менен")
        right.addWidget(self.input_I)

        btn_apply = QPushButton("U жана Rфиксин колдонуу")
    ...
    def apply_params(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "U жана R фикс үчүн сандык маанилерди киргизиңиз.")
            return
    ...
    def check_I(self):
        ...
        if abs(I_user - I_true) <= tol:
            ...
            self.lbl_result.setText(f"❌ Туура эмес. Туура I ≈ {I_true:.3f} А (чектөө ±{tol:.3f}).")

    def show_I(self):
        self.input_I.setText(f"{self.circuit.I:.3f}")
        self.lbl_result.setText("Учурдагы R менен туура I көрсөтүлдү.")
