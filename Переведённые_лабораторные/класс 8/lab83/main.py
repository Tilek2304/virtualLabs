...
class LabCurrentImprovedApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык — Ырастар чынында агымдын күчү (жакшартылган)")
        ...
        right.addWidget(QLabel("<b>Ырастар чынында агымдын күчү</b>"))
        info = QLabel(
            "Булак чыңдыгын жана ырастай туташтырылган чырак каршылыктарын киргизиңиз.\n"
            "Чынча чогултуп, агымдын ар бир аймагында бирдей экенин текшериңиз: I = U / (R1+R2+...)."
        )
        info.setWordWrap(True)
        right.addWidget(info)

        self.input_U = QLineEdit(); self.input_U.setPlaceholderText("U (В)")
        self.input_Rs = QLineEdit(); self.input_Rs.setPlaceholderText("Каршылыктар үтүүнөн ажыратылган (Ом), мисалы: 10,15,20")
        self.input_I = QLineEdit(); self.input_I.setPlaceholderText("I (А) — сиздин эсептөөңүз")
    ...
    def build(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "Чыңдыгын U жана каршылыктар тизмегин үтүүнөн ажыратуу менен киргизиңиз.")
            return
        self.circuit.set_params(U, Rs)
        self.lbl_result.setText("Чынча чогултулду. Амметрди карап, I эсептеңиз.")
    ...
    def check(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Ката", "I үчүн сандык маанини киргизиңиз.")
            return
        ...
        if abs(I_user - I_true) <= tol:
            ...
            self.lbl_result.setText(f"❌ Туура эмес. Туура I = {I_true:.3f} A (чектөө ±{tol:.3f}).")
    ...
    def random_experiment(self):
        ...
        self.circuit.set_params(U, Rs)
        self.input_I.clear()
        self.lbl_result.setText("Жаңы таажрыйба түзүлдү.")
