...
class LabCmpHeatApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык — Катуу денелердин салыштырмалык жылуулук сыйымдуулугу")
        ...
        right.addWidget(QLabel("<b>Катуу денелердин салыштырмалык жылуулук сыйымдуулугу</b>"))
        info = QLabel(
            "Ыстуу цилиндрди сууга салыңыз («Цилиндрди салуу» баскычы), андан кийин «Аралаштыруу»ды басыңыз.\n"
            "Акыркы температураны T ченеп алып, формула боюнча c₂ ды эсептеңиз:\n"
            "c₂ = (c₁·m₁·(T − t₁)) / (m₂·(t₂ − T)). Бирдиктери: Дж/(г·°C)."
        )
        ...
        self.input_c1 = QLineEdit(); self.input_c1.setPlaceholderText("c1 суунун (Дж/(г·°C))")
        ...
        right.addSpacing(8)
        right.addWidget(QLabel("<b>Окуучунун жоопу</b>"))
        self.input_T = QLineEdit(); self.input_T.setPlaceholderText("T (°C) — акыркы температура")
        self.input_c2 = QLineEdit(); self.input_c2.setPlaceholderText("c2 (Дж/(г·°C)) — өзүңүз эсептеңиз")
    ...
    def mix(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Ката", "m1, t1, m2, t2, c1 үчүн сандык маанилерди киргизиңиз.")
            return
        ...
        if self.cal.t_final is None:
            QMessageBox.information(self, "Инфо", "Параметрлер туура эмес. Киргизүүнү текшериңиз.")
        else:
    ...
    def check_c2(self):
        ...
        if self.cal.t_final is None:
            QMessageBox.information(self, "Инфо", "Адегечү аралаштырып T табыңыз.")
            return
        try:
            c2_user = float(self.input_c2.text())
        except Exception:
            QMessageBox.warning(self, "Ката", "c2 үчүн сандык маанини киргизиңиз.")
            return
        ...
        if abs(c2_user - c2_true) <= tol:
            ...
            self.lbl_result.setText(f"❌ Туура эмес. Туура c₂ ≈ {c2_true:.3f} Дж/(г·°C) (чектөө ±{tol:.3f}).")
    ...
    def show_answers(self):
        ...
        self.lbl_result.setText("T жана c₂ үчүн туура маанилер көрсөтүлдү.")
