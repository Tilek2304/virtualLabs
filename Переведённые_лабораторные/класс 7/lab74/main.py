...
class MenzurkaWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(info_x, info_y + 20, f"Бөлүнүүлөр: {self.divisions}")
        painter.drawText(info_x, info_y + 40, f"V1 (көрсөтүлгөн): {self.V1:.1f} мл")
...
class Lab04App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык №4 — Суюктукту ыгызып өлчөө ыкмасы")
        ...
        # Controls on right
        lbl_title = QLabel("<b>Таажрыйба: Суюктукту ыгызып өлчөө</b>")
        right_layout.addWidget(lbl_title)

        self.lbl_instructions = QLabel(
            "Денени мензуркага салып, деңгээлдердин айырмасы аркылуу дененин көлөмүн аныктаңыз.\n"
            "V1, V2 жана V = V2 - V1 маанилерин миллилитрде киргизиңиз."
        )
        self.lbl_instructions.setWordWrap(True)
        right_layout.addWidget(self.lbl_instructions)

        # Buttons for animation
        btn_lower = QPushButton("Төмөн түшүрүү")
        btn_lower.clicked.connect(self.menzurka.start_lower)
        btn_raise = QPushButton("Жогорулату")
        ...
        self.input_V1.setPlaceholderText("V1 киргизиңиз, мл")
        self.input_V2 = QLineEdit()
        self.input_V2.setPlaceholderText("V2 киргизиңиз, мл")
        self.input_V = QLineEdit()
        self.input_V.setPlaceholderText("V = V2 - V1 киргизиңиз, мл")
    ...
    def _on_check(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "V1, V2 жана V талааларына сандык маанилерди киргизиңиз.")
            return
        ...
        if ok_V1:
            ...
            lines.append(f"❌ V1 туура эмес. Туура V1: {true_V1:.1f} мл (чектөө ±{tol_level:.2f} мл).")
        if ok_V2:
            lines.append("✅ V2 туура эсептелди.")
        else:
            lines.append(f"❌ V2 туура эмес. Туура V2: {true_V2:.1f} мл (чектөө ±{tol_level:.2f} мл).")
        if ok_V:
            lines.append("✅ Дененин V көлөмү туура эсептелди.")
        else:
            lines.append(f"❌ Дененин V туура эмес. Туура V: {true_V:.1f} мл (чектөө ±{tol_V:.2f} мл).")
