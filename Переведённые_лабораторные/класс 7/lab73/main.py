...
class ScalesWidget(QFrame):
    """
    Рычажтык терезди чизмелейт: опора, балка, табактар. Гирлерди сүйрөөгө уруксат берет.
    Сол жана оң табактардагы массаларды кайтарып берет.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimumSize(900, 420)
        self.center = QPointF(450, 180)  # позиция опоры (экран координаттары)
        self.beam_length = 360  # балканын толук узундугу пикселдерде
        self.beam_angle = 0.0
    ...
    def paintEvent(self, event):
        ...
        for wgt in self.weights:
            ...
            painter.drawText(pos.x()-10, pos.y()+4, f"{wgt.mass} г")
            if wgt.on_plate:
...
class Lab03App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык №3 — Рычажтык терези (массаны киргизүү)")
        ...
        lbl_title = QLabel("<b>Таажрыйба: Рычажтык терези</b>")
        right_layout.addWidget(lbl_title)

        self.lbl_info = QLabel(
            "Гирлерди челекке сүйрөңүз. Максат: белгисиз дененин массасын аныктоо.\n"
            "Тек гана белгисиз дененин массасын киргизиңиз (г)."
        )
        self.lbl_info.setWordWrap(True)
        right_layout.addWidget(self.lbl_info)

        # Показ текущих сумм масс на чашах
        self.lbl_masses = QLabel("m_left = 0 г\nm_right = 0 г")
        right_layout.addWidget(self.lbl_masses)

        # Поле ввода для ученика (только масса)
        right_layout.addSpacing(6)
        right_layout.addWidget(QLabel("<b>Белгисиз дененин массасын киргизүү</b>"))
        self.input_unknown = QLineEdit()
        self.input_unknown.setPlaceholderText("Белгисиз дененин массасын киргизиңиз (г)")
    ...
    def update_values(self):
        ...
        self.lbl_masses.setText(f"m_left = {m_left:.1f} г\nm_right = {m_right:.1f} г")

    def check_answer(self):
        try:
            user_unknown = float(self.input_unknown.text())
        except Exception:
            QMessageBox.warning(self, "Ката", "Массаны сан түрүндө киргизиңиз.")
            return
        m_left, _ = self.scales.sum_masses()
        tol = max(0.5, m_left * 0.05)  # допуск: 5% или 0.5 g
        if abs(user_unknown - m_left) <= tol:
            self.lbl_result.setText("✅ Белгисиз дененин массасы туура эсептелди.")
        else:
            self.lbl_result.setText(f"❌ Туура эмес. Сол челекке болгон суммарлуу масса: {m_left:.1f} г (чектөө ±{tol:.2f} г).")
    ...
    def reset_experiment(self):
        ...
        self.lbl_result.setText("Эксперимент кайра башталды.")
