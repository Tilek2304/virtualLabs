...
class Lab02App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык №2 — Катар ыкмасы менен өлчөө")
        ...
        self.lbl_params = QLabel(
            f"Линейканын узундугу: <b>{self.ruler_length_mm} мм</b>\n"
            "Шарларды чыкылдатуу менен кошуңуз, чычкандын жардамы менен сүйрөңүз.\n"
            """Тегиз катар алуу үчүн "Катарды тегиздөө" баскычын басыңыз."""
        )
        ...
        self.input_L.setPlaceholderText("L киргизиңиз, мм")
        self.input_N = QLineEdit()
        self.input_N.setPlaceholderText("N киргизиңиз, даана")
        self.input_d = QLineEdit()
        self.input_d.setPlaceholderText("d киргизиңиз, мм (L/N)")
    ...
    def on_check(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "L, N, d талааларына сандык маанилерди киргизиңиз.")
            return
        ...
        if ok_L:
            ...
            lines.append(f"❌ L туура эмес. Туурасы: {true_L:.1f} мм (толук ±{tol_L:.2f} мм).")
        if ok_N:
        ...
        if ok_d:
            ...
            lines.append(f"❌ d туура эмес. Туурасы: {true_d:.3f} мм (толук ±{tol_d:.3f} мм).")
...
class Lab02App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык №2 — Катарный метод измерения размеров")
        # Увеличенная ширина окна
        self.setMinimumSize(1200, 600)
        ...
        right_layout.addSpacing(8)
        right_layout.addWidget(QLabel("<b>Жооптарды киргизүү</b>"))
        self.input_L = QLineEdit()
        self.input_L.setPlaceholderText("Киргизиңиз L, мм")
        self.input_N = QLineEdit()
        self.input_N.setPlaceholderText("Киргизиңиз N, шт")
        self.input_d = QLineEdit()
        self.input_d.setPlaceholderText("Киргизиңиз d, мм (L/N)")
        right_layout.addWidget(self.input_L)
        right_layout.addWidget(self.input_N)
        right_layout.addWidget(self.input_d)

        btn_check = QPushButton("Текшер")
        btn_check.clicked.connect(self.on_check)
        btn_show = QPushButton("Жооп көрсөт")
        btn_show.clicked.connect(self.on_show)
        right_layout.addWidget(btn_check)
        right_layout.addWidget(btn_show)
    ...
    def on_check(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "Сандык маанилерди киргизиңиз в поля L, N, d.")
            return
        true_L = self.balls_widget.measured_length_mm()
        true_N = self.balls_widget.count_balls()
        true_d = (true_L / true_N) if true_N > 0 else 0.0
        ...
        if ok_L:
            ...
            lines.append(f"❌ L Туура эмес. Туура: {true_L:.1f} мм (Чектөө ±{tol_L:.2f} мм).")
        if ok_N:
            lines.append("✅ N рассчитано верно.")
        else:
            lines.append(f"❌ N Туура эмес. Туура: {true_N} шт.")
        if ok_d:
            lines.append("✅ d рассчитано верно.")
        else:
            lines.append(f"❌ d Туура эмес. Туура: {true_d:.3f} мм (Чектөө ±{tol_d:.3f} мм).")
        self.lbl_result.setText("\n".join(lines))
    ...
    def on_show(self):
        ...
        self.input_N.setText(str(true_N))
        self.input_d.setText(f"{true_d:.4f}")
        self.lbl_result.setText("Показаны правильные маанилер.")
