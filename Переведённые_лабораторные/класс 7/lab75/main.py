...
class MenzurkaWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(body_cx - 18, body_cy + 4, "дене")
        ...
        painter.drawText(info_x, info_y + 0, f"Толук көлөм: {self.total_volume} мл")
        painter.drawText(info_x, info_y + 18, f"Бөлүнүүлөр: {self.divisions}")
        painter.drawText(info_x, info_y + 36, f"V1 (көрсөтүлгөн): {self.V1:.1f} мл")
...
class ScalesWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        for w in self.weights:
            ...
            painter.drawText(pos.x()-10, pos.y()+4, f"{w.mass} г")
            if w.on_plate:
...
class LabDensityApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык — Катуу денелердин тыгыздыгы")
        ...
        right_col.addWidget(QLabel("<b>Катуу денелердин тыгыздыгы</b>"))
        info = QLabel("Дененин тыгыздыгын аныктаңыз: массаны (m) терезде жана көлөмдү (V) мензуркада өлчөңүз.\n"
                      "m, V жана эсептелген тыгыздык ρ = m / V (г/мл) киргизиңиз.")
        info.setWordWrap(True)
        right_col.addWidget(info)

        # показ текущих измерений
        self.lbl_current = QLabel("m_left = 0 г\nV_current = 0.0 мл")
        right_col.addWidget(self.lbl_current)

        # поля ввода
        right_col.addSpacing(6)
        right_col.addWidget(QLabel("<b>Жыйынтыктарды киргизүү</b>"))
        self.input_m = QLineEdit(); self.input_m.setPlaceholderText("m киргизиңиз, г")
        self.input_V = QLineEdit(); self.input_V.setPlaceholderText("V киргизиңиз, мл")
        self.input_rho = QLineEdit(); self.input_rho.setPlaceholderText("ρ = m / V киргизиңиз, г/мл")
        right_col.addWidget(self.input_m)
        right_col.addWidget(self.input_V)
        right_col.addWidget(self.input_rho)

        # кнопки
        btn_update = QPushButton("Маалыматтарды жаңыртуу")
        ...
        btn_reset = QPushButton("Экспериментти кайра баштоо")
        ...
        btn_immerse = QPushButton("Денени сууга салуу")
        ...
        right_col.addWidget(QLabel("<b>Өлчөө таблицасы</b>"))
    ...
    def update_values(self):
        ...
        self.lbl_current.setText(f"m_left = {m_left:.1f} г\nV_current = {V_current:.2f} мл")
    ...
    def check_answers(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "m, V, ρ талааларына сандык маанилерди киргизиңиз.")
            return
        ...
        if ok_m:
            ...
            lines.append(f"❌ m туура эмес. Туура m: {true_m:.2f} г (чектөө ±{tol_m:.2f} г).")
        if ok_V:
            lines.append("✅ V туура эсептелди.")
        else:
            lines.append(f"❌ V туура эмес. Туура V: {true_V:.2f} мл (чектөө ±{tol_V:.2f} мл).")
        if ok_rho:
            lines.append("✅ ρ туура эсептелди.")
        else:
            lines.append(f"❌ ρ туура эмес. Туура ρ: {true_rho:.3f} г/мл (чектөө ±{tol_rho:.3f}).")
    ...
    def add_to_table(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "Кошуу үчүн сандык маанилерди киргизиңиз.")
            return
