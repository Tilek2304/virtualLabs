...
class LabArchimedesApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораториялык — Архимед күчү")
        ...
        right_col.addWidget(QLabel("<b>Таажрыйба: Архимед күчү</b>"))
        info = QLabel(
            "Денеге таасир этүүчү күчүнү аба жана сууда өлчөңүз.\n"
            "F_A = F_air - F_water. Теориялык: F_A = ρ·g·V (суунун ρ = 1.0 г/мл)."
        )
        info.setWordWrap(True)
        right_col.addWidget(info)

        # текущие показания
        self.lbl_state = QLabel("Абалы: дене стол сотында")
        ...
        self.input_Fair = QLineEdit(); self.input_Fair.setPlaceholderText("F_air киргизиңиз, Н")
        self.input_Fwater = QLineEdit(); self.input_Fwater.setPlaceholderText("F_water киргизиңиз, Н")
        self.input_FA = QLineEdit(); self.input_FA.setPlaceholderText("F_A = F_air - F_water киргизиңиз, Н")
        self.input_V = QLineEdit(); self.input_V.setPlaceholderText("Дене көлөмүнүн V киргизиңиз, мл")
        ...
        btn_attach = QPushButton("Денени ээгелесе")
    ...
    def toggle_submerge(self):
        if not self.object_attached:
            QMessageBox.information(self, "Инфо", "Адегечү денени динамометрге ээгелесеңиз.")
            return
    ...
    def measure_air(self):
        if not self.object_attached:
            QMessageBox.information(self, "Инфо", "Денени ээгелесе, андан кийин «Абада өлчөө» баскычын басыңыз.")
            return
        self._update_dynamometer(self.F_air_true)
        QMessageBox.information(self, "Өлчөө", f"Өлчөнгөн F_air = {self.F_air_true:.3f} Н")
        self._update_ui()

    def measure_water(self):
        if not self.object_attached:
            QMessageBox.information(self, "Инфо", "Денени ээгелесе жана аны сууга салыңыз.")
            return
        if not self.object_submerged:
            QMessageBox.information(self, "Инфо", "Адегечү денени сууга салыңыз (Сууга салуу / Чыгаруу баскычы).")
            return
        self._update_dynamometer(self.F_water_true)
        QMessageBox.information(self, "Өлчөө", f"Өлчөнгөн F_water = {self.F_water_true:.3f} Н")
        self._update_ui()
    ...
    def _update_ui(self):
        state_text = "ээгелген" if self.object_attached else "стол сотында"
        sub_text = "сууга салынды" if self.object_submerged else "сууга салынбады"
        self.lbl_state.setText(f"Абалы: дене {state_text}, {sub_text}")
    ...
    def check_answers(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "F_air, F_water, F_A жана V талааларына сандык маанилерди киргизиңиз.")
            return
        ...
        lines = []
        if ok_Fair:
            lines.append("✅ F_air туура эсептелди.")
        else:
            lines.append(f"❌ F_air туура эмес. Туура: {true_Fair:.3f} Н (чектөө ±{tol_F:.3f} Н).")
        if ok_Fwater:
            lines.append("✅ F_water туура эсептелди.")
        else:
            lines.append(f"❌ F_water туура эмес. Туура: {true_Fwater:.3f} Н (чектөө ±{tol_F:.3f} Н).")
        if ok_FA:
            lines.append("✅ F_A туура эсептелди.")
        else:
            lines.append(f"❌ F_A туура эмес. Туура: {true_FA:.3f} Н (чектөө ±{max(0.02*true_FA,0.01):.3f} Н).")
        if ok_V:
            lines.append("✅ V туура эсептелди.")
        else:
            lines.append(f"❌ V туура эмес. Туура көлөм: {true_V:.1f} мл (чектөө ±{tol_V:.2f} мл).")
    ...
    def show_answers(self):
        ...
        QMessageBox.information(self, "Маанилер көрсөтүлдү", "Туура маанилер талааларга толтурулду.")
    ...
    def reset_experiment(self):
        ...
        self.lbl_readings.setText("F_air = — Н\nF_water = — Н\nV (мл) = —")
        self.lbl_state.setText("Абалы: дене стол сотында")
    ...
    def random_experiment(self):
        ...
        self._update_dynamometer(0.0)
        self.input_Fair.clear(); self.input_Fwater.clear(); self.input_FA.clear(); self.input_V.clear()
        QMessageBox.information(self, "Жаңы таажрыйба", f"Жаңы дене түзүлдү: V = {self.V_body} мл, m = {self.mass_body_g} г")
