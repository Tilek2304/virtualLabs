...
class LabInternalResApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ЭДС и внутреннее сопротивление источника")
        ...
        right.addWidget(QLabel("<b>Параметры источника</b>"))
        ...
        right.addSpacing(6)
        right.addWidget(QLabel("<b>Управление нагрузкой</b>"))
        ...
        btn_open = QPushButton("Измерить E (разомкнутая цепь)")
        btn_open.clicked.connect(self.measure_open)
        btn_load = QPushButton("Измерить при нагрузке (имитация)")
