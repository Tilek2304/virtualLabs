...
def flux_change(L, dI=0.0, dB=0.0, area=1.0):
    # простая модель: Φ = N * B * A; здесь L используется как N (число витков) для удобства
    # возвращает ΔΦ при изменении B или тока (если считать B ~ I)
    return L * dB * area
...
class MeterWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        ang = -60 + (frac + 1) * 60  # map -1..1 -> -60..60
        r = math.radians(ang)
...
class LabInductionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Электромагнитная индукция — катушка + магнит + гальванометр")
        ...
        right.addWidget(QLabel("Число витков N"))
        self.input_N = QLineEdit(); self.input_N.setPlaceholderText("например 200")
        right.addWidget(self.input_N)
        right.addWidget(QLabel("Площадь витка A (m^2)"))
        self.input_A = QLineEdit(); self.input_A.setPlaceholderText("например 0.01")
        right.addWidget(self.input_A)
        right.addWidget(QLabel("Индукция магнита B0 (T)"))
        self.input_B0 = QLineEdit(); self.input_B0.setPlaceholderText("например 0.8")
        right.addWidget(self.input_B0)
        right.addWidget(QLabel("Сопротивление нагрузки R (Ω)"))
        self.input_R = QLineEdit(); self.input_R.setPlaceholderText("например 100")
        right.addWidget(self.input_R)

        right.addSpacing(6)
        right.addWidget(QLabel("<b>Управление магнитом</b>"))
        right.addWidget(QLabel("Скорость движения магнита (px/s)"))
        ...
        self.input_dphi = QLineEdit(); self.input_dphi.setPlaceholderText("ΔΦ (Wb) — изменение потока")
        self.input_dt = QLineEdit(); self.input_dt.setPlaceholderText("Δt (s) — время изменения")
        self.input_E = QLineEdit(); self.input_E.setPlaceholderText("ЭДС E (V) — ваш расчёт")
        ...
        btn_start = QPushButton("Запустить движение")
        ...
        btn_check = QPushButton("Проверить E")
    ...
    def apply_params(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения N, A, B0 и R.")
            return
        self.ind_widget.set_params(N=N, area=A, B0=B0, R_load=R)
        self.lbl_feedback.setText("Параметры применены. Нажмите «Запустить движение» или «Измерить».")
        self._update_ui()

    def start_motion(self):
        # движение уже управляется слайдером; просто даём пользователю знать
        self.lbl_feedback.setText("Движение магнита идёт (управляйте скоростью ползунком).")
        # не меняем скорость здесь

    def stop_motion(self):
        self.ind_widget.set_magnet_speed(0.0)
        self.slider_speed.setValue(0)
        self.lbl_feedback.setText("Движение остановлено.")
        self._update_ui()
    ...
    def check(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения ΔΦ, Δt и ваш расчёт E.")
            return
        if abs(dt_user) < 1e-9:
            QMessageBox.information(self, "Инфо", "Δt слишком мал для корректного расчёта.")
            return
        ...
        lines = []
        if ok_user:
            lines.append("✅ Ваш расчёт E соответствует вычислению по измерениям.")
        else:
            lines.append(f"❌ Ваш E не совпадает с расчётом по измерениям. E_расчёт = {E_calc:.6e} V.")
        if ok_model:
            lines.append("✅ Измерение близко к модельной ЭДС (по симуляции).")
        else:
            lines.append(f"❌ Измерение отличается от модельной ЭДС = {E_model:.6e} V (допуск ±{tol_model:.6e}).")
    ...
    def show_answer(self):
        ...
        self.lbl_feedback.setText("Показаны правильные значения по модели (аппроксимация за шаг).")
        self._update_ui()
    ...
    def random_experiment(self):
        ...
        self.lbl_feedback.setText("Случайный эксперимент сгенерирован. Нажмите «Измерить» или управляйте скоростью.")
        self._update_ui()
