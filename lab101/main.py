...
class LabInductionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораторная работа №16: Электромагнитная индукция")
    ...
    def setup_ui(self):
        ...
        task_l = QVBoxLayout()
        task_l.addWidget(QLabel("1. Нажмите 'Старт'."))
        task_l.addWidget(QLabel("2. Когда стрелка отклонится максимально, нажмите 'Пауза'."))
        task_l.addWidget(QLabel("3. Запишите показания и нажмите 'Проверить'."))
        ...
        self.lbl_v = QLabel("Скорость: 20 м/с")
        self.slider_v.valueChanged.connect(lambda v: self.lbl_v.setText(f"Скорость: {v} м/с"))
        ...
        btn_pause = QPushButton("Пауза / Продолжить")
        ...
        # 3. Результаты
        res_g = QGroupBox("Анализ данных")
        res_l = QVBoxLayout()
        
        self.in_umax = QLineEdit()
        self.in_umax.setPlaceholderText("U_max (снятое на паузе)")
        
        btn_add = QPushButton("Проверить и добавить")
        btn_add.clicked.connect(self.check_and_add)
        
        res_l.addWidget(QLabel("Макс. напряжение:"))
        ...
        self.table.setHorizontalHeaderLabels(["N (витков)", "v (скорость)", "U_max (мВ)"])
    ...
    def check_and_add(self):
        ...
        if real_u == 0:
            QMessageBox.warning(self, "Ошибка", "Сначала запустите эксперимент!")
            return

        if abs(u_user - real_u) <= real_u * 0.15:
            QMessageBox.information(self, "Верно", "✅ Значение считано верно! Добавлено в таблицу.")
            ...
            QMessageBox.warning(self, "Ошибка", f"❌ Неверно. Гальванометр показал {real_u:.1f}.")
