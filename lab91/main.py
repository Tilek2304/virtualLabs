...
class LabInductanceApp(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Лабораторная — Индуктивность катушки (L = ΔΦ / ΔI)")
        ...
        self.input_L_true = QLineEdit(); self.input_L_true.setPlaceholderText("L (Гн) — (для модели, оставить пустым для случайного)")
        self.input_I1 = QLineEdit(); self.input_I1.setPlaceholderText("I1 (A) — начальный ток")
        self.input_I2 = QLineEdit(); self.input_I2.setPlaceholderText("I2 (A) — конечный ток")
        self.input_dt = QLineEdit(); self.input_dt.setPlaceholderText("Δt (с) — время изменения тока")
        self.input_R = QLineEdit(); self.input_R.setPlaceholderText("R нагрузки (Ω)")
        ...
        right.addSpacing(6)
        right.addWidget(QLabel("<b>Поля ученика (введите свои измерения/расчёты)</b>"))
        self.input_E_meas = QLineEdit(); self.input_E_meas.setPlaceholderText("ЭДС E (В) — измеренное (или заполнить имитацией)")
        self.input_di = QLineEdit(); self.input_di.setPlaceholderText("ΔI (A) — изменение тока (I2−I1)")
        self.input_dt_meas = QLineEdit(); self.input_dt_meas.setPlaceholderText("Δt (с) — время изменения (повторно)")
        self.input_L_user = QLineEdit(); self.input_L_user.setPlaceholderText("L (Гн) — ваш расчёт")
        ...
        btn_change = QPushButton("Изменить ток (симуляция ΔI)")
        btn_change.clicked.connect(self.change_current)
        btn_measure = QPushButton("Измерить (имитация показаний)")
        btn_measure.clicked.connect(self.measure)
        btn_check = QPushButton("Проверить L")
    ...
    def apply_params(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения I1, I2, Δt и R (L опционально).")
            return
        # применяем в модели
        self.coil_widget.set_state(I_now=I2, I_prev=I1, L_true=L, R_load=R)
        self.lbl_feedback.setText("Параметры применены. Нажмите «Изменить ток» для симуляции изменения.")
        self.update_labels()
    ...
    def change_current(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите I1, I2, Δt и R перед симуляцией.")
            return
        ...
        self.lbl_feedback.setText("Симуляция изменения тока выполнена. Нажмите «Измерить» для имитации показаний.")
        self.update_labels()
    ...
    def measure(self):
        ...
        if E is None:
            QMessageBox.information(self, "Инфо", "Сначала выполните симуляцию изменения тока.")
            return
    ...
    def check(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите числовые значения E, ΔI, Δt и ваш расчёт L.")
            return
        if abs(dI_user) < 1e-9:
            QMessageBox.information(self, "Инфо", "ΔI слишком мал для корректного расчёта L.")
            return
        ...
        lines = []
        if ok_calc:
            lines.append("✅ Ваш расчёт L соответствует вычисленному по измерениям.")
        else:
            lines.append(f"❌ Ваш L не совпадает с расчётом по измерениям. L_расчёт = {L_calc:.6f} H.")
        if ok_true:
            lines.append("✅ Измерение близко к истинному значению индуктивности (по модели).")
        else:
            lines.append(f"❌ Измерение отличается от истинного L (модель): L_ист = {L_true:.6f} H (допуск ±{tol:.6f}).")
    ...
    def show_answer(self):
        ...
        if abs(dI) < 1e-9:
            QMessageBox.information(self, "Инфо", "ΔI слишком мал или не задан.")
            return
        E = self.coil_widget.E_ind
        L_calc = - E * dt / dI
