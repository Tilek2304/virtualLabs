...
class MenzurkaWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(info_x, info_y + 18, f"Бөлүктөр: {self.divisions}")
        # КОТОРМО: V1 (показан) -> V1 (көрсөтүлдү)
        painter.drawText(info_x, info_y + 36, f"V1 (көрсөтүлдү): {self.V1:.1f} мл")
        
        if self.state in (1,2,3) or self.anim_t > 0:
            V2 = self.V1 + self.V_body
            # КОТОРМО: V2 (итог) -> V2 (жыйынтык)
            painter.drawText(info_x, info_y + 54, f"V2 (жыйынтык): {V2:.1f} мл")
        else:
            painter.drawText(info_x, info_y + 54, f"V2 (жыйынтык): —")
...
class LabDensityApp(QWidget):
    def __init__(self):
        super().__init__()
        # КОТОРМО: Лабораторная — Плотность твёрдых тел -> Лабораториялык иш — Катуу нерселердин тыгыздыгы
        self.setWindowTitle("Лабораториялык иш — Катуу нерселердин тыгыздыгы")
        ...
        # Правая панель
        # КОТОРМО: Плотность твёрдых тел -> Катуу нерселердин тыгыздыгы
        right_col.addWidget(QLabel("<b>Катуу нерселердин тыгыздыгы</b>"))
        
        # КОТОРМО: Инструкция
        info = QLabel(
            "Нерсенин тыгыздыгын аныктаңыз: массасын (m) таразада, көлөмүн (V) мензурка менен ченеңиз.\n"
            "m, V жана эсептелген тыгыздыкты ρ = m / V (г/мл) киргизиңиз."
        )
        info.setWordWrap(True)
        right_col.addWidget(info)

        # КОТОРМО: m_left -> m_сол, V_current -> V_азыркы
        self.lbl_current = QLabel("m_сол = 0 г\nV_азыркы = 0.0 мл")
        ...
        self.input_m = QLineEdit(); self.input_m.setPlaceholderText("m маанисин киргизиңиз, г")
        self.input_V = QLineEdit(); self.input_V.setPlaceholderText("V маанисин киргизиңиз, мл")
        self.input_rho = QLineEdit(); self.input_rho.setPlaceholderText("ρ = m / V маанисин киргизиңиз, г/мл")
        ...
        # КОТОРМО: Добавить в таблицу -> Таблицага кошуу
        btn_add = QPushButton("Таблицага кошуу")
        ...
        # КОТОРМО: Погрузить тело -> Нерсени сууга салуу
        btn_immerse = QPushButton("Нерсени сууга салуу")
        ...
        # КОТОРМО: Таблица измерений -> Өлчөөлөрдүн таблицасы
        right_col.addWidget(QLabel("<b>Өлчөөлөрдүн таблицасы</b>"))
        self.table = QTableWidget(0, 3)
        self.table.setHorizontalHeaderLabels(["m, г", "V, мл", "ρ, г/мл"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        right_col.addWidget(self.table)

        # КОТОРМО: Средняя -> Орточо
        self.lbl_stats = QLabel("Орточо ρ: —    σ: —")
    ...
    def update_values(self):
        ...
        # КОТОРМО: m_left -> m_сол
        self.lbl_current.setText(f"m_сол = {m_left:.1f} г\nV_азыркы = {V_current:.2f} мл")
    ...
    def check_answers(self):
        try:
            ...
            # КОТОРМО: Ошибка -> Ката
            QMessageBox.warning(self, "Ката", "m, V, ρ талааларына сан маанилерин киргизиңиз.")
            return
        ...
        lines = []
        if ok_m:
            lines.append("✅ m туура эсептелди.")
        else:
            lines.append(f"❌ m туура эмес. Туурасы: {true_m:.2f} г ( piela ±{tol_m:.2f} г).")
        if ok_V:
            lines.append("✅ V туура эсептелди.")
        else:
            lines.append(f"❌ V туура эмес. Туурасы: {true_V:.2f} мл ( piela ±{tol_V:.2f} мл).")
        if ok_rho:
            lines.append("✅ ρ туура эсептелди.")
        else:
            lines.append(f"❌ ρ туура эмес. Туурасы: {true_rho:.3f} г/мл ( piela ±{tol_rho:.3f}).")
    ...
    def add_to_table(self):
        try:
            ...
            QMessageBox.warning(self, "Ката", "Кошуудан мурун сан маанилерин киргизиңиз.")
            return
        # ... (код добавления в таблицу без изменений) ...
        self._update_stats()
    ...
    def _update_stats(self):
        ...
        if vals:
            ...
            self.lbl_stats.setText(f"Орточо ρ: {avg:.4f} г/мл    σ: {sigma:.4f}")
        else:
            self.lbl_stats.setText("Орточо ρ: —    σ: —")
