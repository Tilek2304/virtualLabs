...
class Lab01App(QWidget):
    def __init__(self):
        super().__init__()
        # КОТОРМО: Лабораторная №1 -> №1 Лабораториялык иш
        # Цена деления мензурки -> Мензурканын бөлүгүнүн баасы
        self.setWindowTitle("№1 Лабораториялык иш — Мензурканын бөлүгүнүн баасы")
        ...
        # КОТОРМО: Параметры мензурки -> Мензурканын параметрлери
        lbl_title = QLabel("<b>Мензурканын параметрлери</b>")
        lbl_title.setAlignment(Qt.AlignLeft)
        info_layout.addWidget(lbl_title)

        # КОТОРМО: Полный объём -> Толук көлөмү
        self.lbl_total = QLabel(f"Толук көлөмү: <b>{self.V_total} мл</b>")
        # КОТОРМО: Число делений -> Бөлүктөрдүн саны
        self.lbl_divs = QLabel(f"Бөлүктөрдүн саны: <b>{self.N}</b>")
        # КОТОРМО: Хинт
        self.lbl_hint = QLabel("Суюктуктун деңгээли шкалада көрсөтүлгөн.\nБөлүктүн баасын жана көлөмүн эсептеңиз.")
        ...
        # КОТОРМО: Введите цену деления -> Бөлүктүн баасын киргизиңиз
        self.input_price.setPlaceholderText("Бөлүктүн баасын киргизиңиз (мл)")
        input_layout.addWidget(self.input_price)

        self.input_volume = QLineEdit()
        # КОТОРМО: Введите объём жидкости -> Суюктуктун көлөмүн киргизиңиз
        self.input_volume.setPlaceholderText("Суюктуктун көлөмүн киргизиңиз (мл)")
    ...
    def check_answers(self):
        try:
            ...
            # КОТОРМО: Ошибка -> Ката
            QMessageBox.warning(self, "Ката", "Эки талаага тең сан маанилерин киргизиңиз.")
            return
        ...
        if ok_price:
            ...
            msg_lines.append("✅ Бөлүктүн баасы туура эсептелди.")
        else:
            # КОТОРМО: Неверно. Правильная... -> Туура эмес. Туура баасы...
            msg_lines.append(f"❌ Туура эмес. Бөлүктүн туура баасы: {self.price:.2f} мл.")

        if ok_volume:
            msg_lines.append("✅ Суюктуктун көлөмү туура эсептелди.")
        else:
            msg_lines.append(f"❌ Туура эмес. Туура көлөмү: {self.V_liquid} мл.")

        self.lbl_result.setText("\n".join(msg_lines))

    def show_answer(self):
        self.lbl_result.setText(
            f"Бөлүктүн туура баасы: {self.price:.2f} мл\n"
            f"Суюктуктун туура көлөмү: {self.V_liquid} мл"
        )
